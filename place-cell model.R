...
Nruns <- 20 #how many runs to run if not plotting anything
...
par(mfrow=c(1,4))
#par(mfrow=c(Ndays,Ntrials*pln),mar=c(2,2,2,1))
...
#        weights <- wres
        ...
        if (plot_trajectories&((day==1&trial==1)|(day==5&trial==4)))
        {
          #plot the maze
        ...
        if (plot_cognitive_maps&((day==1&trial==1)|(day==5&trial==4))){
          #plot the maze
...
if(pln > 0.5){
  ...
  all_pc = as.data.frame(cbind(latency,dist[,2],target_quadrant[,2],opposite_quadrant[,2],wall_zone[,2]))
  colnames(all_pc)=c("day","latency","dist","target_quadrant","opposite_quadrant","wall_zone")
